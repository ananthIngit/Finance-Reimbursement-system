...
const FinanceProfile = () => {
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            ...
            alert("Finance photo updated!");
        } catch (error) {
            console.error("Photo upload failed", error);
        }
    };

    if (loading) return <div className="p-10 text-center text-gray-500 dark:text-gray-400">Loading Finance Profile...</div>;
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-6 transition-colors duration-300 bg-gray-100 dark:bg-gray-900 relative">
            ...
            <div className="rounded-2xl shadow-xl overflow-hidden max-w-lg w-full transition-colors duration-300 bg-white dark:bg-gray-800">
                ...
                <div className="h-32 relative bg-emerald-600 dark:bg-emerald-700">
                    <button
                        onClick={() => navigate('/finance-dashboard')}
                        className="absolute top-4 left-4 text-white px-3 py-1 rounded-full text-sm font-medium transition bg-black/20 hover:bg-black/40"
                    >
                        ← Back to Dashboard
                    </button>
                </div>
                ...
                <div className="px-8 pb-8">
                    <div className="relative flex justify-center -mt-16 mb-6">
                        <div className="relative h-32 w-32 rounded-full border-4 shadow-lg group overflow-hidden border-white bg-gray-200 dark:border-gray-800 dark:bg-gray-700">
                            
                            {profile.profile_picture ? (
                                <img src={profile.profile_picture} alt="Profile" className="h-full w-full object-cover" />
                            ) : (
                                <div className="h-full w-full flex items-center justify-center text-4xl font-bold uppercase bg-emerald-100 text-emerald-500 dark:bg-emerald-900/50 dark:text-emerald-300">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <p className="text-xs font-bold text-emerald-500 uppercase tracking-widest mt-1">Finance Admin</p>
                    </div>
                    ...
                    <div className="mt-8 text-center space-x-4">
                        {isEditing ? (
                            <>
                                <button onClick={handleSave} className="px-6 py-2 rounded-lg shadow font-medium text-white bg-emerald-600 hover:bg-emerald-700">Save Changes</button>
                            ...
                            <button onClick={() => setIsEditing(true)} className="font-medium text-sm border px-6 py-2 rounded-lg transition text-emerald-600 border-emerald-600 dark:text-emerald-400 dark:border-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20">
